...
import { deleteAttachmentsFromCloudinary, emitEvent } from "../utils/features.js";
...
const newGroupChat = TryCatch(async (req, res, next) => {
    ...
    // will create an emit function currently it will not do anything and we will create that function on features.js
...
const addMembers = TryCatch(async (req, res, next) => {
    ...
    // my app doesn't have any limit for group members.
...
const leaveGroup = TryCatch(async (req, res, next) => {
    ...
    // if admin leaves the group the first one in the array of members will be crowned as an admin.
    if (chat.creator.toString() === req.user.toString()) {
...
const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    if (files.length > 10) return next(new ErrorHandler("You can send upto 10 files at a time.", 400));

    // let's find the chat id and the user sent it
    const [chat, myself] = Promise.all([
        Chat.findById(chatId),
        User.findById(req.user, "name")
    ]);
    ...
    // files to be uploaded on cloudinary
    const attachments = [];
    ...
    // the message that will be sending to socket.io
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    // collecting all public ids of attachments to delete from cloudinary
